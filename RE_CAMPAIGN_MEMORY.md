...
- `last completed step`: taught the compare/finalize runtime helpers to emit a
  ...
  a temporary raw payload automatically, while the no-savestate fallback now
  has a checked-in JSON pad-input scaffold that the wrapper can replay through
  `PCSX-Redux` instead of relying on a purely manual menu path
- `next recommended step`: keep using
  ...
  lives elsewhere. If no savestate is available yet, tune
  `decomp/evidence/opcode_0x80_runtime_input_plan.json` and rerun with
  `-UseDefaultInputPlan` so the same scripted wrapper drives the cold-boot
  menu path. Once the run reaches `0x800BFBB8`,
...
- `do not forget`: store the raw dump files and the compare report under
  ...
  `.codex_tmp/pcsx-redux` is temporary, preserve any tuned input-plan JSON
  edits as checked-in handoff evidence if the no-savestate path is used, use
  the recorder helper instead of leaving evidence only in emulator UI state,
  and refresh the scaffolded compare/support files instead of creating
  parallel ad-hoc notes
...
- `2026-04-30`: turned the `0x80` runtime handoff into a checked-in scaffold
  ...
  [`decomp/evidence/opcode_0x80_runtime_support.md`](decomp/evidence/opcode_0x80_runtime_support.md),
  [`decomp/verification/finalize_runtime_observation.py`](decomp/verification/finalize_runtime_observation.py)
- `2026-04-30`: added a reusable runtime-observation recorder so the next
  `PCSX-Redux` pass can append dump paths, breakpoint hits, dispatches, and
  mutation notes into the checked-in packet without hand-editing JSON, while
  the support note now renders timestamps and free-form runtime notes. Links:
  [`decomp/verification/record_runtime_observation.py`](decomp/verification/record_runtime_observation.py),
  [`decomp/evidence/opcode_0x80_runtime_capture_plan.md`](decomp/evidence/opcode_0x80_runtime_capture_plan.md),
  [`decomp/evidence/opcode_0x80_runtime_observation.json`](decomp/evidence/opcode_0x80_runtime_observation.json),
  [`decomp/verification/finalize_runtime_observation.py`](decomp/verification/finalize_runtime_observation.py)
- `2026-04-30`: taught the runtime-observation recorder to import changed
  compare-report rows back into `table_mutations`, so a real `PCSX-Redux`
  capture can promote rewritten `0x80-0x82` handlers into the checked-in
  packet without manual transcription. Links:
  [`decomp/verification/record_runtime_observation.py`](decomp/verification/record_runtime_observation.py),
  [`decomp/evidence/opcode_0x80_runtime_capture_plan.md`](decomp/evidence/opcode_0x80_runtime_capture_plan.md),
...
- `2026-05-01`: added a checked-in no-savestate fallback for the `0x80`
  runtime tie-breaker by teaching the wrapper/Lua path to replay a JSON
  pad-input plan, making the breakpoint callbacks safer with queued main-loop
  work, and seeding a conservative starter plan for future cold-boot runs.
  Links:
  [`decomp/verification/run_opcode_0x80_runtime_capture.ps1`](decomp/verification/run_opcode_0x80_runtime_capture.ps1),
  [`decomp/verification/pcsx_redux_opcode_0x80_capture.lua`](decomp/verification/pcsx_redux_opcode_0x80_capture.lua),
  [`decomp/evidence/opcode_0x80_runtime_input_plan.json`](decomp/evidence/opcode_0x80_runtime_input_plan.json),
